...
import { useCreatePreset, useUpdatePreset } from "../preset-api";
...
export const NewPresetForm: FC<NewPresetFormProps> = ({ preset, onSubmit }) => {
  const isEdit = preset.id.length > 0;
  ...
  const handleSubmit = async (values: PresetFormValues) => {
    try {
      if (isEdit && preset) {
        ...
        await createPreset.mutateAsync({ ...preset, ...values });
